...
void min_capacity_test() {
    ...
    CHECK(cb1.size() == 0);
    CHECK(cb1.capacity().capacity() == 10);
    CHECK(cb1.capacity().min_capacity() == 10);
    CHECK(cb2[0] == 1);
    CHECK(cb2.size() == 10);
    CHECK(cb2.capacity() == 10);
    CHECK(cb2.capacity().min_capacity() == 5);
    CHECK(cb3[0] == 1);
    CHECK(cb3.size() == 5);
    CHECK(cb3.capacity() == 20);
    CHECK(cb3.capacity().min_capacity() == 10);
    CHECK(cb1.capacity().min_capacity() <= cb1.internal_capacity());
    CHECK(cb2.capacity().min_capacity() <= cb2.internal_capacity());
    CHECK(cb3.capacity().min_capacity() <= cb3.internal_capacity());
...
void capacity_control_test() {
    ...
    CHECK(c1.min_capacity() == 0);
    CHECK(c2.capacity() == 20);
    CHECK(c2.min_capacity() == 5);
    CHECK(c3.capacity() == 20);
    CHECK(c3.min_capacity() == 5);

    c1 = c2;

    CHECK(c1.capacity() == 20);
    CHECK(c1.min_capacity() == 5);
}
...
void specific_constructors_test() {
    ...
    CHECK(cb1.capacity().min_capacity() == 0);
    CHECK(cb1.internal_capacity() == 0);
    CHECK(cb1.size() == 0);
    ...
    CHECK(cb2.capacity().min_capacity() == 0);
    CHECK(cb2.size() == 3);
}
...
void shrink_to_fit_test() {
    ...
    CHECK(cb.size() == 3);
    CHECK(cb.capacity() == 1000);
    CHECK(internal_capacity >= cb.internal_capacity());
}
...
// test main
TEST_CASE("space_optimized_test", "[circular_buffer]")
{
